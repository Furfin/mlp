...
Matrix<T>::Matrix(std::initializer_list<T> const &items)
  ...
  if (fmod((items.size()), sqrt(items.size())) != 0)
    error_ = true;
...
template <typename T> Matrix<T>::~Matrix() { DeleteMatrix(); }

template <typename T> void Matrix<T>::CreateMatrix() {
  matrix_ = new T *[rows_];
  for (auto i = 0; i < rows_; i++)
    matrix_[i] = new T[cols_]();
}

template <typename T> int Matrix<T>::GetCols() const { return cols_; }

template <typename T> int Matrix<T>::GetRows() const { return rows_; }

template <typename T> void Matrix<T>::SetRows(const int rows) {
  if (rows < 1)
...
template <typename T> void Matrix<T>::SetCols(const int cols) {
  if (cols < 1)
...
template <typename T> void Matrix<T>::ResizeMatrix(int rows, int cols) {
...
template <typename T> void Matrix<T>::DeleteMatrix() {
  if (matrix_ != nullptr) {
    for (auto i = 0; i < rows_; i++)
      delete[] matrix_[i];
    delete[] matrix_;
...
template <typename T> bool Matrix<T>::IsEqualSize(const Matrix &other) {
  return (cols_ == other.cols_ && rows_ == other.rows_);
}

template <typename T> bool Matrix<T>::IsEqualMatrix(const Matrix &other) {
...
template <typename T> void Matrix<T>::CopyMatrix(Matrix const &other) {
...
template <class T> bool Matrix<T>::operator==(const Matrix &other) {
...
template <class T> void Matrix<T>::MulMatrix(const Matrix<T> &other) {
